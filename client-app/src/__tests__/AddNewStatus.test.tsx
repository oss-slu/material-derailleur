import { validateRequiredField, validateDate, validateImages, validateStatusForm } from '../Components/AddNewStatus';
...
describe('AddNewStatus Validation', () => {
    ...
    describe('validateDate', () => {
        ...
        it('should return error for invalid date format', () => {
            ...
            expect(result).toBe('Invalid date format. Please use YYYY-MM-DD format.');
        });
    });
    ...
    describe('validateImages', () => {
        it('should return error for too many images', () => {
            ...
            expect(result.errors.general).toBe('You can upload up to 5 images only.');
        });

        it('should return error for invalid file type', () => {
            const mockImages = [createMockFile('test.pdf', 'application/pdf', 1000000)];
            const result = validateImages(mockImages);
            expect(result.isValid).toBe(false);
            expect(result.errors['image_0']).toContain('is not a valid image type');
        });

        it('should return error for file too large', () => {
            const mockImages = [createMockFile('large.jpg', 'image/jpeg', 3 * 1024 * 1024)];
            const result = validateImages(mockImages);
            expect(result.isValid).toBe(false);
            expect(result.errors['image_0']).toContain('exceeds the 2MB size limit');
        });
