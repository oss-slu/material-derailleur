...
function getCfg() {
  const STORAGE_BACKEND = (process.env.STORAGE_BACKEND || 'azure').toLowerCase(); // 'local' | 'azure'
  const UPLOADS_DIR = process.env.UPLOADS_DIR || 'uploads';
  const PUBLIC_BASE_URL = process.env.PUBLIC_BASE_URL || 'http://localhost:5050';
...
async function uploadLocal(file: Express.Multer.File, filename: string): Promise<string> {
...
async function uploadAzureViaStorage(file: Express.Multer.File, filename: string): Promise<string> {
...
const fetchImageFromCloud = async (url: string): Promise<string | null> => {
  try {
    ...
    if (STORAGE_BACKEND === 'local' && url.startsWith(`${PUBLIC_BASE_URL}/uploads/`)) {
